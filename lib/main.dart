...
// REMOVED: import 'database_helper.dart'; // No longer needed
...
class _MainPageState extends State<MainPage> {
  String _username = '';
  String _name = '';
  // Groups now stores the data directly from the server, including unread/last message time
  ...
  void initState() {
    super.initState();
    _syncProfileAndGroups(); // Directly sync with server
  }
  
  // 🌟 MODIFIED: Only syncs with server (no local DB involvement)
  Future<void> _syncProfileAndGroups({bool forceLoading = true}) async {
    ...
    try {
      final url = Uri.parse("https://$SERVER_IP/profile_and_groups"); // 🌟 NEW SERVER ENDPOINT
      final response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
      ...
      if (data['success']) {
        if (mounted) {
          // 1. Update profile from server
          await storage.write(key: 'username', value: data['username']);
          await storage.write(key: 'name', value: data['name']);

          // 2. Groups loaded directly from server response (including unread info)
      ...
      if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Network error. Could not sync groups.')));
      }
  ...
  // 🌟 MODIFIED: Navigation to ChatPage is the same, but full sync is required on return
  void _navigateToChat(Map<String, dynamic> group) async {
    ...
    // Always perform a full sync after returning from ChatPage
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () async {
              // Read current name from storage before navigating (in case it was updated)
              final currentName = await storage.read(key: 'name') ?? _name;
              await Navigator.push(
                context, 
              ...
              child: _groups.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        // NEW: Get unread count and time directly from server payload
        ...
        children: [
          FloatingActionButton.extended(
            ...
            onPressed: () async {
              ...
              // Force sync to update group list after creation
              _syncProfileAndGroups(forceLoading: true);
            },
          ...
          FloatingActionButton.extended(
            ...
            onPressed: () async {
              ...
              // Force sync to update group list after joining
              _syncProfileAndGroups(forceLoading: true);
            },
  ...
  Future<void> _updateProfile() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(data['message'])));
        if (data['success']) {
          // Update local storage only, the MainPage will refetch
...
class _ChatPageState extends State<ChatPage> {
  ...
  // Messages are stored only in this state variable
  List<Map<String, dynamic>> messages = [];
  bool _isLoading = false;
  Timer? _timer;
  final ScrollController _scrollController = ScrollController();
  
  // 🌟 NEW: Track the latest message time for incremental sync
  ...
  void initState() {
    super.initState();
    _syncMessages(forceLoading: true).then((_) { 
        _scrollToBottom();
        // Mark as read after initial sync
        _markGroupAsRead(); 
    });
  ...
  // 🌟 NEW SERVER ENDPOINT: Mark group as read (to reset unread count)
  Future<void> _markGroupAsRead() async {
      try {
          final url = Uri.parse("https://$SERVER_IP/mark_read"); 
          await http.post(
              url,
              headers: {"Content-Type": "application/json"},
              body: json.encode({
  ...
  // 🌟 MODIFIED: Sync now fetches ONLY incoming messages (no outbox needed)
  Future<void> _syncMessages({bool isPolling = false, bool forceLoading = false}) async {
    if (forceLoading) setState(() => _isLoading = true);
    ...
    try {
      ...
      var response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
        // Use the latest time from the current UI list for incremental sync
        body: json.encode({"token": widget.token, "last_synced_time": _latestMessageTime}), 
      );
      ...
      if (data['success']) {
        ...
        if (serverMessages.isNotEmpty || forceLoading) {
           // Append new messages, or replace the list on initial load/full refresh
          final newMessages = forceLoading ? serverMessages : messages + serverMessages;
          
          if (mounted) {
            bool shouldScroll = serverMessages.isNotEmpty;
            
            // Find the absolute latest message time in the full list
            if (newMessages.isNotEmpty) {
                 newMessages.sort((a, b) => (a['time'] as String).compareTo(b['time'] as String));
                 _latestMessageTime = newMessages.last['time'] as String;
            }
      ...
      if (!isPolling && mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Network Error. Could not fetch messages.')));
      }
    } finally {
        if (mounted && forceLoading) setState(() => _isLoading = false);
    }
  ...
  // 🌟 MODIFIED: Send message now goes DIRECTLY to the server
  Future<void> sendMessage() async {
    String text = messageController.text.trim();
    if (text.isEmpty) return;
    
    // Optimistic UI update: Show a "pending" message
    final tempMessage = {
      'sender': widget.username,
      'message': text,
      'time': DateTime.now().toUtc().toIso8601String(),
      'status': 'sending', // Custom status for online-only
    };
    setState(() {
    ...
    try {
        ...
        if (data['success']) {
            // Success: Remove temp message, and sync to get the server's definitive message
            setState(() {
                messages.remove(tempMessage); 
            });
            await _syncMessages(); // Fetch the new message from the server
            _markGroupAsRead(); // Mark as read after a successful send/sync
        } else {
            // Failure: Update temp message status
            if (mounted) {
                 setState(() {
                    messages.lastWhere((m) => m == tempMessage)['status'] = 'failed';
                 });
                 ScaffoldMessenger.of(context).showSnackBar(
                     SnackBar(content: Text('Message failed to send: ${data['message']}')));
            }
        }
    } catch (e) {
         if (mounted) {
             // Network failure: Update temp message status
             setState(() {
                messages.lastWhere((m) => m == tempMessage)['status'] = 'failed';
             });
             ScaffoldMessenger.of(context).showSnackBar(
                 SnackBar(content: Text('Network error: Could not send message.')));
         }
    } 
  }
  
  // 🌟 MODIFIED: GIF sender now uses the direct sendMessage/sync pattern
  Future<void> _sendSpecialMessage(String content) async {
    // This effectively uses the same logic as sendMessage, but for the GIF URL
    final tempMessage = {
      'sender': widget.username,
      'message': content,
      'time': DateTime.now().toUtc().toIso8601String(),
  ...
  Future<void> leaveGroup() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (data['success']) {
          // No local deletion needed, simply pop and the MainPage will re-sync
          ...
          _isLoading
            ...
            : Expanded(
                child: RefreshIndicator(
                  onRefresh: () => _syncMessages(forceLoading: true), // Full refresh on pull
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // Check for custom status 'sending' or 'failed'
                      bool isPending = msg['status'] == 'sending'; 
                      ...
                      return Container(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: isMe 
                                ? (isPending || isFailed ? secondaryColor.withOpacity(0.5) : primaryColor.withOpacity(0.8))
                                : Colors.grey[300],
                                    ...
                                    if (isMe && isPending)
                                        Padding(
                                          ...
                                          child: Icon(
                                            Icons.access_time, 
                                            size: 10, 
                                            color: Colors.yellow[800] // Yellow for sending
                                          ),
                                        ),
                                    if (isMe && isFailed)
                                        Padding(
                                          ...
                                          child: Icon(
                                            Icons.error_outline, 
                                            size: 10, 
                                            color: Colors.red[800] // Red for failed
                                          ),
                                        )
                                ],
                              ),
