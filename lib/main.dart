...
import 'package:giphy_get/giphy_get.dart'; // ⚠️ NEW IMPORT
            ...
            child: Column(
              children: [
                ...
                TextField(
                  ...
                  obscureText: true,
                  decoration: const InputDecoration(
                    // Escaped $ for literal character in string
                    hintText: '123\$\%^gkf',
                    border: OutlineInputBorder(),
...
class _MainPageState extends State<MainPage> {
  // Store all group data, including is_creator
  ...
  Future<void> fetchGroups() async {
    try {
      ...
      if (data['success']) {
        ...
        if (mounted) {
          setState(() {
            // Map group data to include all fields
            groups = userGroups
        ...
        child: groups.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  // Show (Admin) next to group name if is_creator is true
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      onTap: () async {
                        // Use await and check for result after chat page closes
...
class _ChatPageState extends State<ChatPage> {
  ...
  // Modified to handle both text and GIF messages
  Future<void> sendMessage({String? text, String? gifUrl}) async {
    ...
    // Format message for GIF if a URL is provided
    if (gifUrl != null) {
    ...
    try {
      ...
      if (data['success']) {
        ...
        // Since we are polling, we can just clear the text field and let the timer fetch the new message.
        // However, forcing an immediate fetch for better UX.
        await fetchMessages(); 
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context)
  ...
  // ⚠️ NEW: Function to open GiphyGet UI
  void _openGiphySearch() async {
    // Check if the widget is still mounted before proceeding
    if (!mounted) return;
    
    // Call the GiphyGet function
    GiphyGif? gif = await GiphyGet.getGif(
      context: context,
      apiKey: GIPHY_API_KEY, // Use the API key constant
      lang: GiphyLanguage.english,
      // You can customize the UI here if needed
      // showGiphyAttribution: false,
      // theme: ThemeData.dark(), 
    );

    // If a GIF was selected, send its URL as a message
    if (gif != null) {
  ...
  // Delete Group implementation (Admin only)
  ...
  // Show dialog with group options
      ...
      body: Column(
        children: [
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Display Image.network if it's a GIF
                          if (isGif && gifUrl != null) 
                            ...
                            Text(messageText), // Display regular text
          ...
          Container(
            ...
            child: Row(
              children: [
                // NEW: GIF Button to open GiphyGet UI
                IconButton(
                  icon: const Icon(Icons.gif_box),
                  color: Colors.blue[900],
                  onPressed: _openGiphySearch, // Call the new function
                ),
                Expanded(
                  child: TextField(
